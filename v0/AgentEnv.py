...
class CarAgent(agent.Agent):
    ...
    class MyBehav(CyclicBehaviour):
        ...
        def dqn_car(self):
            ...
            dist_csv = open('../csv/v0/Distances_%d_%d_%d.csv' % (Config.EPISODES, Config.MINIBATCH_SIZE, Config.REPLAY_MEMORY_SIZE), 'w')
            dist_csv_writer = csv.writer(dist_csv, delimiter=";")
            dist_csv_writer.writerow(['Episodio', 'Distancia', 'Tipo'])

            acc_csv = open('../csv/v0/Acc_%d_%d_%d.csv' % (Config.EPISODES, Config.MINIBATCH_SIZE, Config.REPLAY_MEMORY_SIZE), 'w')
            acc_csv_writer = csv.writer(acc_csv, delimiter=";")
            acc_csv_writer.writerow(['Episodio', 'Precision'])

            loss_csv = open('../csv/v0/Loss_%d_%d_%d.csv' % (Config.EPISODES, Config.MINIBATCH_SIZE, Config.REPLAY_MEMORY_SIZE), 'w')
            ...
            plt.savefig('../graficas/v0/Distances_%d_%d_%d.png' % (Config.EPISODES, Config.MINIBATCH_SIZE, Config.REPLAY_MEMORY_SIZE))
            plt.clf()
            plt.plot(accX, accY)
            plt.title('model accuracy')
            plt.ylabel('accuracy')
            plt.xlabel('epoch')
            plt.savefig('../graficas/v0/Acc_%d_%d_%d.png' % (Config.EPISODES, Config.MINIBATCH_SIZE, Config.REPLAY_MEMORY_SIZE))
            plt.clf()
            plt.plot(accX, lossY)
            plt.title('model loss')
            plt.ylabel('loss')
            plt.xlabel('epoch')
            plt.savefig('../graficas/v0/Loss_%d_%d_%d.png' % (Config.EPISODES, Config.MINIBATCH_SIZE, Config.REPLAY_MEMORY_SIZE))
